--キャノン・ソルジャー
function c11384280.initial_effect(c)
	--damage
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(11384280,0))
	e1:SetCategory(CATEGORY_DAMAGE)
	e1:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCost(c11384280.cost)
	e1:SetTarget(c11384280.target)
	e1:SetOperation(c11384280.operation)
	c:RegisterEffect(e1)
--
local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_ACTIVATE)
	e2:SetCost(c11384280.cost)
	e2:SetTarget(c11384280.target)
	e2:SetOperation(c11384280.operation)
	c:RegisterEffect(e2)

--
end
function c11384280.cost(e,tp,eg,ep,ev,re,r,rp,chk,obg_cn,obi)
local g1=obg_cn[1]
local cannon_co=g1:GetFirst()
	if chk==0 then return Duel.CheckReleaseGroup(tp,nil,1,nil) end
if cannon_co then
Duel.Release(cannon_co,REASON_COST)
else
	local sg=Duel.SelectReleaseGroup(tp,nil,1,1,nil)
	Duel.Release(sg,REASON_COST)
end
end
function c11384280.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetTargetPlayer(1-tp)
	Duel.SetTargetParam(500)
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,500)
end
function c11384280.operation(e,tp,eg,ep,ev,re,r,rp)
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	Duel.Damage(p,d,REASON_EFFECT)
end
