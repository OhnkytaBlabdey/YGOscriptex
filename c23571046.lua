...
e2:SetCondition(c23571046.condition)
e2:SetTarget(c23571046.target)
e2:SetOperation(c23571046.operation)
...
function c23571046.operation(e,tp,eg,ep,ev,re,r,rp,obg,obi)

	if not Duel.IsExistingMatchingCard(c23571046.cfilter,tp,LOCATION_MZONE,0,1,nil) then return end
	local c=e:GetHandler()
if obi then
local hg_pos=obi[1]
if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,0,tp,tp,false,false,hg_pos)>0 then
		local e1=Effect.CreateEffect(c)
